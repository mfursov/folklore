...
fun spell_summon_water_elm(level: Int, ch: CHAR_DATA) {
    if (is_affected(ch, Skill.summon_water_elemental)) {
    ...
    for (gch in Index.CHARS) {
        if (gch.pcdata == null && IS_AFFECTED(gch, AFF_CHARM) && gch.master == ch && gch.pIndexData.vnum == MOB_VNUM_ELM_WATER) {
            i++
            if (i > 2) {
    ...
    elm.max_hit = if (pcdata == null)
        ...
        Math.min(5 * pcdata.perm_hit + 2000, 30000)
    elm.hit = elm.max_hit
    ...
    val af = Affect()
    af.type = Skill.summon_water_elemental
    af.level = level
    af.duration = 24
