...
fun spell_hurricane(level: Int, ch: CHAR_DATA) {
    ...
    for (vch in ch.room.people) {
        ...
        if (is_safe_spell(ch, vch, true) || ch.pcdata == null && vch.pcdata == null
        ...
        if (saves_spell(level, vch, DT.Other)) {
            damage(ch, vch, dam / 2, Skill.hurricane, DT.Other, true)
        } else {
            damage(ch, vch, dam, Skill.hurricane, DT.Other, true)
        }
    }
}