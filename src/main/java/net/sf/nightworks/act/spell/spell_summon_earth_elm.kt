...
import net.sf.nightworks.MOB_VNUM_ELM_EARTH
...
fun spell_summon_earth_elm(level: Int, ch: CHAR_DATA) {
    if (is_affected(ch, Skill.summon_earth_elemental)) {
    ...
    val elm = create_mobile(get_mob_index_nn(MOB_VNUM_ELM_EARTH))
    ...
    elm.max_hit = if (pcdata == null)
        ...
        Math.min(2 * pcdata.perm_hit + 400, 30000)
    elm.hit = elm.max_hit
    ...
    val af = Affect()
    af.type = Skill.summon_earth_elemental
    af.level = level
    af.duration = 24
