...
import net.sf.nightworks.MOB_VNUM_ELM_LIGHT
...
fun spell_summon_light_elm(level: Int, ch: CHAR_DATA) {
    if (is_affected(ch, Skill.summon_lightning_elemental)) {
    ...
    for (gch in Index.CHARS) {
        if (gch.pcdata == null && IS_AFFECTED(gch, AFF_CHARM) && gch.master == ch &&
    ...
    val elm = create_mobile(get_mob_index_nn(MOB_VNUM_ELM_LIGHT))
    ...
    elm.max_hit = if (pcdata == null)
        ...
        Math.min(10 * pcdata.perm_hit + 4000, 30000)
    elm.hit = elm.max_hit
    ...
    val af = Affect()
    af.type = Skill.summon_lightning_elemental
    af.level = level
    af.duration = 24
